...
1. Разметить диск (мой вариант, не как в Arch Wiki)
    ...
    - Если надо перезаписать весь диск, то создать пустую таблицу разделов GPT
    - EFI-раздел где-либо в пределах первых 2.2 Тб диска (обычно в самом начале), на нём будет или уже есть папка EFI
