...
- Включить AUR
- Настроить локали, чтобы был нормальный порядок символов при сортировке, метрическая система мер, размеры бумаги и формат записи времени (в LibreOffice надо будет менять в его настройках из-за, возможно, того, что у него нет локали `C.UTF-8` и он откатывается на значение `LC_LANG`):
